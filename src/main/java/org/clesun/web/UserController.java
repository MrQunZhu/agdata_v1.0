...
@RestController
@RequestMapping(value = "/dy/user")
public class UserController extends AbsRest<DUser>{

    private Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
    private IDUserService baseService;

    @MethodDescription("List")
    @ResponseBody
    @RequestMapping(method = RequestMethod.GET)
    public Object list(Pagination<DUser> pageinfo, HttpServletRequest request, HttpServletResponse resp) {
        WsResult result = new WsResult();
        WebUtils.setPageDataParameter(request, pageinfo,"_");
        List<DUser> list = new ArrayList<DUser>();
    ...
    public Object get(@PathVariable("id") Long id, HttpServletRequest request, HttpServletResponse resp) {
        WsResult result = new WsResult();
        try {
            DUser obj = baseService.getById(id);
            result.setStatus(Constant.Status.SUCCESS);
            result.setDatas(obj);
    ...
    public Object save(@RequestBody DUser object, HttpServletResponse resp) {
        WsResult result = new WsResult();
        try {
    ...
    public Object update(@RequestBody DUser object,  HttpServletResponse resp) {
        WsResult result = new WsResult();
        try {
            if(isValid(result, null)){
